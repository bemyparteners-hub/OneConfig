...
function initFolderNavigation() {
  ...
  picker.addEventListener('change', () => {
    renderFolderList(Array.from(picker.files || []));
  });
}
  ...
  files.forEach((file) => {
    ...
    if (path.length > 1) {
      for (let i = 1; i < path.length; i++) {
        folders.add(path.slice(0, i).join('/'));
      }
    }
  });

  const sortedFolders = Array.from(folders).sort((a, b) => a.localeCompare(b, 'fr', { sensitivity: 'base' }));

  if (!sortedFolders.length) {
    folderList.innerHTML = '<li class="muted">Aucun sous-dossier trouvé (fichiers masqués).</li>';
    return;
  }

  folderList.innerHTML = sortedFolders.map((folder) => `<li>${folder}</li>`).join('');
}
